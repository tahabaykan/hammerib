...
class PreferredStockMonitor(tk.Tk):
    ...
    def setup_ui(self):
        ...
        self.take_profit_longs_button = ttk.Button(self.button_frame2, text="Take Profit Longs", 
        ...
        self.take_profit_shorts_button = ttk.Button(self.button_frame2, text="Take Profit Shorts", 
        ...
        try:
            ...
            for port in ports:
                try:
                    ...
                    self.ib.connect('127.0.0.1', port, clientId=87, readonly=True)
                    connected = True
        ...
        try:
            ...
            for data in top_tltr_movers:
                tltr_tree.insert("", "end", values=(
                    data['symbol'],
                    f"{data['last']:.2f}",
                    f"{data['daily_change_cents']:.2f}",
                    f"{data['bid']:.2f}" if data['bid'] else "-",
                    f"{data['ask']:.2f}" if data['ask'] else "-",
                    f"{data['spread']:.2f}" if data['spread'] else "-",
                    f"{data['volume']}" if data['volume'] else "-"
                ))
    ...
    def manage_pff_spreads(self):
        ...
        pff_change_percent = safe_float(pff_ticker.changePercent)
        
        # TLT verilerini bul
        tlt_ticker = self.ib.ticker(tlt_contract)
        tlt_last = safe_float(tlt_ticker.last)
        tlt_change_percent = safe_float(tlt_ticker.changePercent)
        ...
        for symbol in position_symbols:
            ...
            daily_change_percent = safe_float(ticker.changePercent)
            ...
            if pos_type == 'LONG':
                ...
                potential_fill_price = bid_price - spread_amount
            else:  # 'SHORT'
            ...
            if pos_type == 'LONG':
                ...
                # Burada relative_to_benchmark_cents >0 olduğunda hisse benchmark'tan iyi
                if relative_to_benchmark_cents > 0:
                    long_candidates.append(stock_data)
            else:  # 'SHORT'
                # SHORT için negatif ayrışma olduğunda cover et (PFF'ten daha kötü performans)
                # Burada relative_to_benchmark_cents <0 olduğunda hisse benchmark'tan kötü
                if relative_to_benchmark_cents < 0:
        ...
        for stock in long_candidates:
            ...
            # Toplam skor: spread_cents + relative_performance_cents (ikisi de pozitif)
            stock['score'] = spread_cents + rel_perf_cents
        ...
        for stock in short_candidates:
            ...
            # Toplam skor: spread_cents + abs(relative_performance_cents) (spread pozitif, relative negatif)
            stock['score'] = spread_cents + abs(rel_perf_cents)
        ...
        for stock in top_candidates:
            ...
            if stock['position_type'] == 'LONG':
                ...
                # Satış fiyatı: Bid - spread'in %15'i
                sell_price = stock['bid'] - spread_amount
                ...
                # Alış fiyatı: Bid + spread'in %15'i
                buy_price = stock['bid'] + spread_amount
        ...
        # Eğer emirler varsa, onay diyaloğuna ilet
        if orders_to_place:
            # Şimdi emirleri send_orders fonksiyonuna ilet
            self.send_orders(orders_to_place)
        else:
    ...
    def take_profit_from_longs(self):
        """
        "Take Profit from Longs" fonksiyonu:
        1. Pozisyonları T-prefs/C-prefs olarak kategorize eder
        2. LONG pozisyonlar için, ask-spread*0.15 fiyatından satış yapmanın ilgili benchmark'ı geçip geçmediğini kontrol eder
        3. Benchmark outperformance'a göre pozisyonları skorlar ve sıralar
        """
        from tkinter import messagebox
        import time
        import pandas as pd
        ...
        # Yükleniyor penceresi
        loading_window = tk.Toplevel(self)
        loading_window.title("Take Profit from Longs")
        loading_window.geometry("300x150")
        loading_window.transient(self)
        loading_window.grab_set()
        ...
        # ETF listeleri
        etf_list = ["PFF", "TLT"]  # Diğer ETF'ler eklenebilir

        # ETF kontratları
        contracts = {}
        for etf in etf_list:
            contracts[etf] = Stock(etf, 'SMART', 'USD')
            self.ib.reqMktData(contracts[etf])

        # PFF ve TLT için değişkenleri hazırla
        pff_contract = contracts["PFF"]
        tlt_contract = contracts["TLT"]
        
        # ETF verilerinin gelmesi için bekle
        progress["value"] = 20
        progress_label.config(text="ETF verileri bekleniyor...")
        loading_window.update()
        time.sleep(2)  # ETF'lerin veri almasını bekle
        
        # PFF ve TLT değerlerini al
        pff_ticker = self.ib.ticker(pff_contract)
        tlt_ticker = self.ib.ticker(tlt_contract)
        ...
        # Sadece long pozisyonları al
        long_positions = [pos for pos in positions if pos.position > 0]
        
        if not long_positions:
            loading_window.destroy()
            messagebox.showinfo("Bilgi", "Satılabilecek LONG pozisyon bulunamadı!")
            return
        
        # TLTR ve DIV sembol listelerini hazırla
        tltr_symbols = []
        div_symbols = []
        
        # CSV'den sembol tiplerini yükle
        try:
            # TLTR sembolleri
            tltr_df = pd.read_csv('final_thg_with_avg_adv.csv')
            if 'Ticker' not in tltr_df.columns:
                tltr_df = normalize_ticker_column(tltr_df)
            tltr_symbols = tltr_df['Ticker'].tolist()
            
            # DIV sembolleri
            div_df = pd.read_csv('optimized_35_extlt.csv')
            if 'Ticker' not in div_df.columns:
                div_df = normalize_ticker_column(div_df)
            div_symbols = div_df['Ticker'].tolist()
        except Exception as e:
            print(f"Sembol listeleri yüklenirken hata: {e}")
            # Hata olsa bile devam et, varsayılan listeler boş olacak
        
        # Pozisyon sembollerini ve verilerini hazırla
        position_symbols = []
        position_data = {}
        ...
        # Semboller için kontratlar oluştur ve market verisi iste
        for symbol in position_symbols:
            contracts[symbol] = Stock(symbol, 'SMART', 'USD')
            self.ib.reqMktData(contracts[symbol])
        
        # Verilerin gelmesi için bekle
        progress["value"] = 80
        progress_label.config(text="Market verileri bekleniyor...")
        loading_window.update()
        time.sleep(2)  # Verilerin gelmesini bekle
        ...
        for symbol in position_symbols:
            ...
            # Potansiyel satış fiyatı (ask - spread*0.15)
            spread_amount = spread_cents * 0.15 / 100
            potential_fill_price = ask_price - spread_amount
            
            # Günlük değişimi cent olarak hesapla (potansiyel fill fiyatı üzerinden)
            if close_price is not None and close_price > 0:
                daily_change_cents = (potential_fill_price - close_price) * 100
            else:
                daily_change_cents = last_price * (daily_change_percent / 100) if daily_change_percent is not None else 0
            ...
            # LONG için pozitif ayrışma olduğunda sat (benchmark'tan daha iyi performans)
            if relative_to_benchmark_cents > 0:
                long_candidates.append(stock_data)
            
            # Tüm adayları ekle (skorlama için)
            all_candidates.append(stock_data)
        ...
        if not long_candidates:
            loading_window.destroy()
            messagebox.showinfo("Bilgi", "Benchmark'ı geçen LONG pozisyon bulunamadı!")
            # Abonelikleri temizle
            for symbol in position_symbols:
                self.ib.cancelMktData(contracts[symbol])
            self.ib.cancelMktData(pff_contract)
        ...
        # En iyi 10 adayı al
        top_candidates = long_candidates[:10]
        
        # Emirleri hazırla
        orders_to_place = []
        
        # En iyi adaylar için emirleri oluştur
        for stock in top_candidates:
            # Long pozisyonu sat
            # Spread'in %15'i kadar içeriden teklif ver
            spread_amount = stock['spread_cents'] * 0.15 / 100
        ...
        if orders_to_place:
            ...
            messagebox.showinfo("Bilgi", "Uygun LONG pozisyon bulunamadı")
            # Abonelikleri temizle
            for symbol in position_symbols:
                self.ib.cancelMktData(contracts[symbol])
            self.ib.cancelMktData(pff_contract)
    ...
    def take_profit_from_shorts(self):
        """
        "Take Profit from Shorts" fonksiyonu:
        1. Pozisyonları T-prefs/C-prefs olarak kategorize eder
        2. SHORT pozisyonlar için, bid+spread*0.15 fiyatından alış yapmanın ilgili benchmark'ı geçip geçmediğini kontrol eder
        3. Benchmark outperformance'a göre pozisyonları skorlar ve sıralar
        """
        from tkinter import messagebox
        import time
        ...
        # Yükleniyor penceresi
        loading_window = tk.Toplevel(self)
        loading_window.title("Take Profit from Shorts")
        loading_window.geometry("300x150")
        loading_window.transient(self)
        loading_window.grab_set()
        ...
        # Sadece short pozisyonları al
        short_positions = [pos for pos in positions if pos.position < 0]
        
        if not short_positions:
            loading_window.destroy()
            messagebox.showinfo("Bilgi", "Kapatılabilecek SHORT pozisyon bulunamadı!")
            return
        ...
        for symbol in position_symbols:
            ...
            # Potansiyel alış fiyatı (bid + spread*0.15)
            spread_amount = spread_cents * 0.15 / 100
            potential_fill_price = bid_price + spread_amount
            ...
            # SHORT için negatif ayrışma olduğunda cover et (benchmark'tan daha kötü performans)
            if relative_to_benchmark_cents < 0:
                short_candidates.append(stock_data)
        ...
        if not short_candidates:
            loading_window.destroy()
            messagebox.showinfo("Bilgi", "Benchmark'ın gerisinde kalan SHORT pozisyon bulunamadı!")
            # Abonelikleri temizle
            for symbol in position_symbols:
                self.ib.cancelMktData(contracts[symbol])
            self.ib.cancelMktData(pff_contract)
        ...
        # En iyi 10 adayı al
        top_candidates = short_candidates[:10]
        ...
        if orders_to_place:
            ...
            messagebox.showinfo("Bilgi", "Uygun SHORT pozisyon bulunamadı")
            # Abonelikleri temizle
            for symbol in position_symbols:
                self.ib.cancelMktData(contracts[symbol])
            self.ib.cancelMktData(pff_contract)
